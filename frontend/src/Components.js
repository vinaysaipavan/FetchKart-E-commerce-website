          ...
          >
            ...
            {Cart[product.id] ? (
              <div className="flex items-center space-x-2">
                <button onClick={() => RemoveFromCart(product.id)} className="px-2 py-1 bg-orange-400 hover:bg-orange-500 rounded-l-lg duration-200"><MdOutlineDeleteForever className="text-[20px]" /></button>
                   <span className="w-4">{Cart[product.id]}</span>
                <button onClick={() => AddToCart(product.id)} className="px-2 py-1 bg-orange-400 hover:bg-orange-500 rounded-r-lg duration-200"><IoMdAdd className="text-[20px]" /></button>
              </div>
                ) : (
              <button className="bottom-4 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition duration-200" onClick={() => AddToCart(product.id)}>
                 Add to Cart
              </button>
              )}
              </div>
